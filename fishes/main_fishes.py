...
from users.main_users import security, verify_admin
...
async def sell_fish(
    ...
    admin_check: dict = Depends(verify_admin)
):
    ...
    csrf_protect.validate_csrf(csrf_token.encode(), request=request)
...
async def delete_fish(fish_id: Annotated[int, Path(ge=0, le=100000)], db: Session = Depends(init_db), admin_check: dict = Depends(verify_admin)):
...
async def get_fish_by_place(cathced: Annotated[str, Path(min_length=3, max_length=20)], db: Session = Depends(init_db)):
...
async def get_fish_by_name(name: Annotated[str, Path(min_length=2, max_length=55)], db: Session = Depends(init_db)):
